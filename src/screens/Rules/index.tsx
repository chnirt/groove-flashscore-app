...
//     * Mọi thắc mắc về công tác tổ chức giải xin liên hệ :
//       * Ban Tổ Chức: Mr. Nguyễn Anh Tuấn – 0984858180
//       * Thư ký: Ms. Võ Ngọc Đan Khuê
//   * Quyết định:
...
const Rules = () => {
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      const settingDocRef = getDocRef('settings', 'rule')
      await addDocument(settingDocRef, settingData)
      Toast.show({
        icon: 'success',
        content: 'Saved',
      })
  ...
  const fetchRules = useCallback(async () => {
    ...
    const systemDocData = await settings.find(
      (setting: any) => setting.id === 'rule'
    )
    setValue(IS_MOCK ? faker.lorem.sentences() : systemDocData.value)
    // setValue(systemDocData.value)
    // setValue(markdown)
  }, [settings])
